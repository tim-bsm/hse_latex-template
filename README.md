...
2. Go to the [`images`-folder](/content/non_tex_files/images/) and replace the two images `company_en.png` and `company_de.png` with the english and german versions of your company's logo. If the english and german version are the same, just use the same image for both files.Moreover, please replace the `signature.jpg` with an image of your own signature.
