...
\subsubsection{Unterunterunterkapitel -- Nicht mehr im Inhaltsverzeichnis}

\enquote{Dies ist ein Beispiel für die Zitierfunktion} \cite[Vgl.][S. 1 ff.]{2026}. Zitate werden in der Reihenfolge ihres Auftretens im Text nummeriert. Nicht verwendete Zitate werden im Literaturverzeichnis unter \enquote{Weitere Quellen} aufgeführt.
...
Wie in \autoref{img:bsp} zu sehen, ist das Logo der \ac{HSE} ein sehr schönes Logo. Das $\lambda$ der \ac{lambda} ist ebenfalls ein sehr schönes Zeichen. Des Weiteren kann auf die Kapitelnummer referenziert werden: \autoref{sec:Unterkapitel} und mit \hyperref[sec:Unterkapitel]{Test Unterkapitel} können eigene Texte für die Referenzen vergeben werden. Mit \todo{Aufgabe für mich} können Aufgaben markiert werden, die noch erledigt werden müssen. \cite{A01,B01}
...
\Acl{dTA} wird großgeschrieben und \ac{dTA} klein, wie es definiert wurde. Eigene Definitionen des Plurals können ebenfalls definiert werden (siehe \texttt{/content/additionals/acronyms.tex}): \acfp{dTA}. Zudem wurde ein \plus\ definiert, welches im Text nicht unterhalb der eigentlichen Schrift positioniert ist (siehe: Test+\plus Test).

\usetable{bsp_rotiert}
\useimage{bsp_rotated}
